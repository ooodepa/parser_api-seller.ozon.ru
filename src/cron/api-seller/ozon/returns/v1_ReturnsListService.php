...
include_once "$HOME/models/OZN_v1_ReturnsList.php";

class v1_ReturnsListService {
    public function executeCron() {
        global $HOME;

        $data = v1_ReturnsListService::getAll();

        $FILE_PATH = "$HOME/data/v1_ReturnsList.json";
        ...
        OZN_v1_ReturnsList::recreateDatabaseAndInsertRows($data);
    }
    ...
    static function getAll() {
        ...
        $result = v1_ReturnsListService::getData($startDate, $endDate);

        $resultArray = [];
        for ($i = 0; $i < count($result); $i++) {
            $data = v1_ReturnsListService::fetchJson__getTransactionsOnPeriod($result[$i]['start'], $result[$i]['end']);
    ...
    static function fetchJson__getTransactionsOnPeriod($dateFrom, $dateTo) {        
        return v1_ReturnsListService::fetchJson([
            "filter" => [
